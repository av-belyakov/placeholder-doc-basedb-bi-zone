...
func (d *DiContainer) KafkaConnecter(ctx context.Context) KafkaConnecter {
	if d.kafka == nil {
		apiKafka, err := kafkaapi.New(
			...
			kafkaapi.WithGroupId(d.Configer().GetKafka().GroupId),
			kafkaapi.WithSASLMechanism(d.Configer().GetKafka().SASLMechanism),
			kafkaapi.WithSSLUserName(d.Configer().GetKafka().SSLUsername),
			kafkaapi.WithSSLPassword(d.Configer().GetKafka().SSLPassword),
			kafkaapi.WithSSLCeFile(d.Configer().GetKafka().SSLCaFile),
			kafkaapi.WithSSLCertFile(d.Configer().GetKafka().SSLCertFile),
			kafkaapi.WithSSLKeyFile(d.Configer().GetKafka().SSLKeyFile),
			kafkaapi.WithSSLServerName(d.Configer().GetKafka().SSLServerName),
